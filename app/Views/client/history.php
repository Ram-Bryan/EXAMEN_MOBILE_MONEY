...
<div class="dashboard-header mb-4">
    <h1 class="h2"><i class="fas fa-history text-orange"></i> Historique des Transactions</h1>
...
<div class="card border-0 shadow-sm" style="border-radius: 16px;">
    <div class="card-header bg-white border-0 py-4 px-4 d-flex justify-content-between align-items-center" style="border-radius: 16px 16px 0 0;">
        <h5 class="mb-0 font-weight-bold"><i class="fas fa-list text-orange me-2"></i> Toutes vos opérations</h5>
        <button class="btn btn-orange-outline btn-sm" onclick="window.location.reload()">
                ...
                <tbody>
                    <?php if (empty($transactions)): ?>
                        <tr>
                            <td colspan="6" class="text-center py-5 text-muted">
                                <i class="fas fa-info-circle fa-2x mb-3 text-orange d-block"></i>
                    ...
                    <?php else: ?>
                        <?php foreach ($transactions as $tx): ?>
                            ...
                            <tr>
                                <td class="ps-4">
                                    <div class="font-weight-bold text-dark" style="font-size: 14px;">#TRX-<?= esc($tx->transaction_id) ?></div>
                                    <div class="text-muted small"><?= date('d/m/Y H:i', strtotime($tx->date_transaction)) ?></div>
                                </td>
                                <td><?= $badge ?></td>
                                <td class="small text-muted font-weight-medium"><?= esc($details) ?></td>
                                ...
                                <td class="text-end pe-4 <?= $color ?> font-weight-bold" style="font-size: 16px;">
